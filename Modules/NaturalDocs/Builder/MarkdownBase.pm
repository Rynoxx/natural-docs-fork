###############################################################################
#
#   Package: NaturalDocs::Builder::MarkdownBase
#
###############################################################################
#
#   A base package for some functionality in the <NaturalDocs::Builder::Markdown> package
#   
#
###############################################################################
...
#
#   Constants: NDMarkupToMD Styles
#
#   These are the styles used with <NDMarkupToMD()>.
#
...
sub BuildContent #(sourceFile, parsedFile)
    ...
    while ($i < scalar @$parsedFile)
        ...
        #            . '<a name="' . $anchor . '" href="#' . $anchor . '">'$headerType
                    $headerType . ' [' . $self->StringToHTML( $parsedFile->[$i]->Title(), ADD_HIDDEN_BREAKS) . '](#' . $anchor . ' \'' . $self->StringToHTML( $parsedFile->[$i]->Title(), ADD_HIDDEN_BREAKS) . "')\n\n";
        #            . '</a>';
        ...
        if (defined $parsedFile->[$i]->Body())
            {
            $output .= $self->NDMarkupToMD( $sourceFile, $parsedFile->[$i]->Body(), $parsedFile->[$i]->Symbol(),
    ...
    my $output = "| Summary | | |\n";
        ...
        while ($index < scalar @$parsedFile)
           ...
           # Add the entry itself.
            ...
            $output .= '[' . $self->StringToHTML( $parsedFile->[$index]->Title(), ADD_HIDDEN_BREAKS) . '](#' . $self->SymbolToHTMLSymbol($parsedFile->[$index]->Symbol()) . " \"" . $self->StringToHTML( $parsedFile->[$index]->Title(), ADD_HIDDEN_BREAKS) . "\") ";
            ...
            if (defined $parsedFile->[$index]->Body())
                {
                $output .= " | " . $self->NDMarkupToMD($sourceFile, $parsedFile->[$index]->Summary(),
            ...
            for (my $i = 0; $i < scalar @$params; $i++)
                ...
                if ($language->TypeBeforeParameter())
                    ...
                    if ($hasTypePrefix)
                        ...
                        $output .= $htmlTypePrefix;
                        };

                    if ($hasType)
                        {
                        $output .= $self->ConvertAmpChars($params->[$i]->Type());
                        };

                    if ($hasNamePrefix)
                        {
                        $output .= $self->ConvertAmpChars($params->[$i]->NamePrefix());
                        };

                    $output .= $self->ConvertAmpChars($params->[$i]->Name());
                    }

                else # !$language->TypeBeforeParameter()
                    {
                    $output .= $self->ConvertAmpChars( $params->[$i]->NamePrefix() . $params->[$i]->Name() );
                    ...
                    if ($hasType || $hasTypePrefix)
                        ...
                        $output .= $self->ConvertAmpChars( $typePrefix . $params->[$i]->Type() );
                        };
                    };

                if ($hasDefaultValuePrefix)
                    {
                    $output .= $self->ConvertAmpChars( $params->[$i]->DefaultValuePrefix() ) . '&nbsp;';
                    };

                if ($hasDefaultValue)
                    {
                    $output .= $self->ConvertAmpChars( $params->[$i]->DefaultValue() );
                    };
                };
    ...
    if (defined $prototype || defined $summary)
        ...
        if (!defined $number)
            ...
            if (defined $summary)
                ...
                $summary = $self->NDMarkupToMD($file, $summary, undef, undef, $type, undef, NDMARKUPTOHTML_TOOLTIP);
                $tooltipHTML .= $summary;
                };
...
sub BuildIndexPages #(TopicType type, NaturalDocs::SymbolTable::IndexElement[] indexSections, string beginIndexPage, string endIndexPage, string beginSearchResultsPage, string endSearchResultsPage) => int
   ...
   for (my $i = 0; $i < 28; $i++)
    ...
    if (!$self->MadeEmptySearchResultsPage())
        {
        my $emptySearchResultsFileName = NaturalDocs::File->JoinPaths( $self->SearchResultsDirectory(), 'NoResults.md' );
    ...
    for (my $i = 0; $i < scalar @$indexHTMLSections; $i++)
        ...
        if ($page != $oldPage)
            ...
            if ($oldPage != -1)
                ...
                print INDEXFILEHANDLE '' . $tooltips . $endIndexPage;
                close(INDEXFILEHANDLE);
                $tooltips = undef;
                };
        ...
        #    . '<td></td>'
        #. '</tr>'
        "| [" . $indexHeadings[$i] . "](" . $indexAnchors[$i] . " '" . $indexHeadings[$i] . "') | |\n|:---|:---|\n"
...
sub BuildIndexLink #(string text, SymbolString symbol, SymbolString package, FileName file, TopicType type, string prototype, string summary, string style) => ( string, string )
    ...
    my $indexHTML = '[' . $text . '](' . $self->MakeRelativeURL( $self->IndexDirectory(), $self->OutputFileOf($file) )
                                         . '#' . $self->SymbolToHTMLSymbol($symbol) . ' "' . $text . '")';
    ...
    my $searchResultHTML = '[' . $text . '](' . $self->MakeRelativeURL( $self->SearchResultsDirectory(), $self->OutputFileOf($file) )
                                         . '#' . $self->SymbolToHTMLSymbol($symbol) . ' "' . $text . '")';
...
sub BuildIndexNavigationBar #(type, page, locations)
    ...
    for (my $i = 0; $i < scalar @indexHeadings; $i++)
        ...
        if (defined $locations->[$i])
            ...
            $output .= '#' . $indexAnchors[$i] . ' "' . $indexHeadings[$i] . '")';
            }
        else
            {
            $output .= $indexHeadings[$i];
...
sub RelativeIndexFileOf #(type, page)
    {
    my ($self, $type, $page) = @_;
    return NaturalDocs::Topics->NameOfType($type, 1, 1) . (defined $page && $page != 1 ? $page : '') . '.md';
    };
...
sub SearchResultsFileOf #(TopicType type, string extra)
    ...
    my $fileName = NaturalDocs::Topics->NameOfType($type, 1, 1) . $extra . '.md';
...
#
#   Function: NDMarkupToMD
#
#   Converts a block of <NDMarkup> to Markdown.
#
#   Parameters:
#
...
#       style - Set to one of the <NDMarkupToMD Styles> or leave undef for general.
#
#   Returns:
#
#       The text in Markdown.
#
sub NDMarkupToMD #(sourceFile, text, symbol, package, type, using, style)
    {
            ...
            sub MakeDescriptionListSymbol #(package, text)
                ...
                "\n| "
                    . ''
                        ...
                        . "[" . $text . "](#" . $self->SymbolToHTMLSymbol($symbol) . " \"" . $text . "\")"
...
#
#   Does nothing, currently only here to prevent any errors from calling the function
#
#   Returns:
#
#       The text.
...
#
#   Creates a Markdown link to a symbol, if it exists.
#
#   Parameters:
#
...
#   Returns:
#
#       The link in Markdown, including tags.  If the link doesn't resolve to anything, returns the Markdown that should be substituted for it.
#
sub BuildTextLink #(target, name, original, package, using, sourceFile)
    ...
    if (defined $symbolTarget)
        ...
        return "[" . $name . "](" . $symbolTargetFile . '#' . $self->SymbolToHTMLSymbol($symbolTarget->Symbol()) . ")";
...
sub BuildURLLink #(target, name)
    ...
    #    {  return "[" . $name . "](" . $target . " \"" . $name . "\")"; }#'<a href="' . $target . '" class=LURL target=_top>' . $name . '</a>';  };
...
sub BuildImage #(sourceFile, mode, target, original)
    ...
    if ($image)
        ...
        if ($mode eq 'inline')
            {
            return "![" . $self->MakeRelativeURL($self->OutputFileOf($sourceFile), $self->OutputImageOf($image), 1) . "](" . $self->MakeRelativeURL($self->OutputFileOf($sourceFile), $self->OutputImageOf($image), 1) . ")";
            ...
            $imageContent .=
            "###Image #" . $imageAnchorNumber . "\n"
            . "*" . ($shortTarget || $target) . "*\n\n"
            . "![" . ($shortTarget || $target) . "](" . $self->MakeRelativeURL($self->OutputFileOf($sourceFile), $self->OutputImageOf($image), 1) . ")";
            #'<blockquote>'
            #. '<div class=CImage>'
